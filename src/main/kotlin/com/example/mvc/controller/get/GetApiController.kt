...
class GetApiController {
    ...
    //@GetMapping(path = ["/hello","/abc"])  => path속성을 넣을 경우 uri경로를 배열로 넣어 hello 와 abc같은 여러개의 주소를 가질수 있음
    @GetMapping("/hello") // Get방식 http://localhost:8080/api/hello uri로 동작함
    fun hello(): String{
        return "hello kotlin"
    }
    //@RequestMapping("request-mapping")  =>  http의 제한없이 get post put delete 동작 가능함 따라서 특정 메서드만 동작하도록 제한을 주기 위해서는 하단의 어노테이션 참고
    ...
    //name, age, address, email 처럼 파라미터로 넘겨야 할 데이터의 수가 많아질 경우(이런경우는 잘 없음)
